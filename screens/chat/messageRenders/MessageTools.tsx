...
export default function MessageTools({ options, onContextPress, onClose, width, position }
    ...
    return (
        <View style={{ alignItems: position === 'left' ? 'flex-start' : 'flex-end', width, paddingHorizontal: 40 }}>
            <ButtonGroup containerStyle={{ width: options.length * 50, height: 66, borderRadius: 10 }}
                buttonContainerStyle={{ backgroundColor: '#444444', padding: 0 }}
                buttons={options.map(i => <Button key={i.code} iconPosition='top'
                    title={i.name} color='#444444' onPress={() => onPress(i.code)}
                    icon={{ name: i.icon, type: i.type, color: '#fff' }}
                    size='sm' titleStyle={{ color: '#fff', fontSize: 12 }}></Button>)}></ButtonGroup>
        </View>
    )
}